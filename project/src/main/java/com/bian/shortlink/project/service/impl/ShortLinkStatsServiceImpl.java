package com.bian.shortlink.project.service.impl;
...
import com.bian.shortlink.project.common.biz.user.UserContext;
import com.bian.shortlink.project.common.convention.exception.ServiceException;
import com.bian.shortlink.project.dao.entity.GroupDO;
import com.bian.shortlink.project.dao.entity.LinkAccessLogsDO;
import com.bian.shortlink.project.dao.entity.LinkAccessStatsDO;
import com.bian.shortlink.project.dao.entity.LinkDeviceStatsDO;
import com.bian.shortlink.project.dao.entity.LinkLocaleStatsDO;
import com.bian.shortlink.project.dao.entity.LinkNetworkStatsDO;
import com.bian.shortlink.project.dao.mapper.LinkGroupMapper;
import com.bian.shortlink.project.dao.mapper.LinkAccessLogsMapper;
import com.bian.shortlink.project.dao.mapper.LinkAccessStatsMapper;
import com.bian.shortlink.project.dao.mapper.LinkBrowserStatsMapper;
import com.bian.shortlink.project.dao.mapper.LinkDeviceStatsMapper;
import com.bian.shortlink.project.dao.mapper.LinkLocaleStatsMapper;
import com.bian.shortlink.project.dao.mapper.LinkNetworkStatsMapper;
import com.bian.shortlink.project.dao.mapper.LinkOsStatsMapper;
import com.bian.shortlink.project.dto.req.ShortLinkGroupStatsAccessRecordReqDTO;
import com.bian.shortlink.project.dto.req.ShortLinkGroupStatsReqDTO;
import com.bian.shortlink.project.dto.req.ShortLinkStatsAccessRecordReqDTO;
import com.bian.shortlink.project.dto.req.ShortLinkStatsReqDTO;
import com.bian.shortlink.project.dto.resp.ShortLinkStatsAccessDailyRespDTO;
import com.bian.shortlink.project.dto.resp.ShortLinkStatsAccessRecordRespDTO;
import com.bian.shortlink.project.dto.resp.ShortLinkStatsBrowserRespDTO;
import com.bian.shortlink.project.dto.resp.ShortLinkStatsDeviceRespDTO;
import com.bian.shortlink.project.dto.resp.ShortLinkStatsLocaleCNRespDTO;
import com.bian.shortlink.project.dto.resp.ShortLinkStatsNetworkRespDTO;
import com.bian.shortlink.project.dto.resp.ShortLinkStatsOsRespDTO;
import com.bian.shortlink.project.dto.resp.ShortLinkStatsRespDTO;
import com.bian.shortlink.project.dto.resp.ShortLinkStatsTopIpRespDTO;
import com.bian.shortlink.project.dto.resp.ShortLinkStatsUvRespDTO;
import com.bian.shortlink.project.service.ShortLinkStatsService;
...
public class ShortLinkStatsServiceImpl implements ShortLinkStatsService {
    ...
    public ShortLinkStatsRespDTO oneShortLinkStats(ShortLinkStatsReqDTO requestParam) {
        ...
        // 基础访问数据（日志聚合；无访问日志时可能为 null，此时用日汇总表合计兜底）
        ...
        if (pvUvUidStatsByShortLink == null) {
            int sumPv = listStatsByShortLink.stream()
                    .map(LinkAccessStatsDO::getPv)
                    .filter(Objects::nonNull)
                    .mapToInt(Integer::intValue)
                    .sum();
            int sumUv = listStatsByShortLink.stream()
                    .map(LinkAccessStatsDO::getUv)
                    .filter(Objects::nonNull)
                    .mapToInt(Integer::intValue)
                    .sum();
            int sumUip = listStatsByShortLink.stream()
                    .map(LinkAccessStatsDO::getUip)
                    .filter(Objects::nonNull)
                    .mapToInt(Integer::intValue)
                    .sum();
            pvUvUidStatsByShortLink = LinkAccessStatsDO.builder()
                    .pv(sumPv)
                    .uv(sumUv)
                    .uip(sumUip)
                    .build();
    ...
    public ShortLinkStatsRespDTO groupShortLinkStats(ShortLinkGroupStatsReqDTO requestParam) {
        ...
        // 基础访问数据（日志聚合；无访问日志时可能为 null，用日汇总合计兜底）
        ...
        if (pvUvUidStatsByGroup == null) {
            int sumPv = listStatsByGroup.stream()
                    .map(LinkAccessStatsDO::getPv)
                    .filter(Objects::nonNull)
                    .mapToInt(Integer::intValue)
                    .sum();
            int sumUv = listStatsByGroup.stream()
                    .map(LinkAccessStatsDO::getUv)
                    .filter(Objects::nonNull)
                    .mapToInt(Integer::intValue)
                    .sum();
            int sumUip = listStatsByGroup.stream()
                    .map(LinkAccessStatsDO::getUip)
                    .filter(Objects::nonNull)
                    .mapToInt(Integer::intValue)
                    .sum();
            pvUvUidStatsByGroup = LinkAccessStatsDO.builder()
                    .pv(sumPv)
                    .uv(sumUv)
                    .uip(sumUip)
                    .build();
